#!/usr/bin/env python
"""
Legacy BoTorch configuration - reverts to original notebook approaches.
...
@dataclass
class Config:
    """Legacy configuration for BoTorch optimization pipeline."""
    
    # Core settings
    random_state: int = 42
    device: str = "auto"
    data_file: str = "data/raw/Antibody_Formulation_Data_Complete_Seed to Batch 2.xlsx"
    ...
    # Model settings - LEGACY MODE
    cv_folds: int = 10  # CHANGE: Basic K-fold instead of Group K-fold  
    min_samples: int = 15
    min_r2: float = 0.2
    legacy_mode: bool = True  # NEW: Flag to enable legacy behaviors
    
    # Optimization settings - LEGACY MODE
    batch_size: int = 6
    mc_samples: int = 128
    num_restarts: int = 10
    reference_point: List[float] = field(default_factory=lambda: [0.0, 0.0, 0.0])  # CHANGE: Fixed reference point
